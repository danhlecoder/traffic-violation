...
def filter_detections_by_roi(
    ...
    if roi and len(roi) >= 3:
        ...
        for det in vehicles:
            bbox = det.get("bbox")
            if bbox and is_bbox_in_roi(bbox, roi, img_width, img_height):
    ...
    for plate in plates:
        ...
        for vehicle in vehicles_in_filtered:
            ...
            if vehicle_bbox and is_bbox_inside_bbox(plate_bbox, vehicle_bbox):
                filtered.append(plate)
                break
    
    # 4. Mũ bảo hiểm: Chỉ hiện khi nằm trong bbox xe máy
    motorcycles_in_filtered = [v for v in vehicles_in_filtered if v.get("class_name") == "motorcycle"]
    for helmet in helmets:
        helmet_bbox = helmet.get("bbox")
        if not helmet_bbox:
            continue
        for motorcycle in motorcycles_in_filtered:
            motorcycle_bbox = motorcycle.get("bbox")
            if motorcycle_bbox and is_bbox_inside_bbox(helmet_bbox, motorcycle_bbox):
